...
def quantize_phase_32level(phi_rad):
    """
    将连续相位量化为 32 级（与 4 级逻辑一致）:
    levels = {0, π/32, 2π/32, ..., 31π/32}
    """
    levels = torch.linspace(0, 31*np.pi/32, steps=32)
    diff = torch.abs(phi_rad[..., None] - levels)
    return levels[torch.argmin(diff, dim=-1)]

def phase_to_etch_depth(phi_rad, t_oxide=T_OXIDE, phi_max=31*np.pi/32):
    """
    32 级映射（与 4 级逻辑一致）:
    φ=0 → etch=676.6nm(全刻), φ=31π/32 → etch=0(不刻)
    """
    phi_clamped = np.clip(phi_rad, 0, phi_max)
    return t_oxide * (1.0 - phi_clamped / phi_max)

# ====== 验证逻辑（以 4 级逻辑为基准） ======
print("验证 4 级逻辑在 32 级中的对应点:")
check_points = [
    (0, "0"),
    (np.pi/4, "π/4"),
    (np.pi/2, "π/2"),
    (3*np.pi/4, "3π/4"),
    (31*np.pi/32, "31π/32 (max)")
]

for phi, name in check_points:
    q = quantize_phase_32level(torch.tensor(phi)).item()
    depth_nm = phase_to_etch_depth(phi) * 1e9
    print(f"  φ={name:<10} → quantized={q/np.pi:.5f}π, etch={depth_nm:.2f} nm")
# %% [markdown]
# ### ***Saving***
# %%
...
def _collect_all_layer_etch_depths(model) -> np.ndarray:
    ...
    for k in phase_keys:
        ...
        phi_quantized = quantize_phase_32level(phi_continuous)
...
print(f"  期望 32 级台阶，实际 unique levels = {len(vals)}")
